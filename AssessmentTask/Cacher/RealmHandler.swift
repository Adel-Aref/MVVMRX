...
protocol LocalStorageProtocol {
    ...
    func getObjects<T: Object>(model: T.Type) -> [T] 
...
class LocalStorage: LocalStorageProtocol {
    fileprivate var realm: Realm? {
        do {
//            print("User Realm User file location: \(String(describing: self.realm?.configuration.fileURL!.path))")
            return try Realm()
        } catch let error {
    ...
    func getObjects<T: Object>(model: T.Type) -> [T] {
        guard let realm: Realm = self.realm else { return [] }
        return realm.objects(model).filter({ !$0.isInvalidated })
    }
...
//          let cached: T? = realm.object()
    ...
    func write<T: Object>(_ objects: [T]?) {
        guard let objects: [T] = objects else { return }
        guard let realm: Realm = self.realm else { return }
